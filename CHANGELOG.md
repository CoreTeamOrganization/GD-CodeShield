...
- **Low-severity findings generate "review" prompts, not "fix" prompts.** For informational notes (e.g. the large-class size note) the Claude prompt is titled "Review SOLID note", frames the content as an observation rather than a problem, and states that "no change needed" is the most likely correct outcome. Medium/High keep the fix framing.
