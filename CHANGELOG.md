...
- **Folder picker redesigned: it grows with its content instead of scrolling inside a fixed box.** The 180px inner scroll view created nested scrolling (wheel events fought between the picker and the page) and never felt right. Expanding a folder now expands the picker; when the column outgrows the window, the content above the START button scrolls as one page. The **START SOLID REVIEW button is pinned to the bottom of the column** and stays visible no matter how far the picker grows.
- **Scrolling now tracks the wheel in real time in all windows.** Scroll views updated their position when a wheel event arrived, but nothing requested a repaint — the new position rendered only when the next unrelated event happened, making scrolling (most visibly the folder picker) feel sticky and jumpy. All four windows now repaint immediately on scroll/drag/mouse-move (`wantsMouseMove` keeps hover highlights live too) — affordable now that draw paths are allocation- and IO-free.
- **UI-wide GC hitches eliminated: styles are now cached at the source.** `BrandTokens.MakeStyle` allocated a new `GUIStyle` on every call, with 120+ call sites across all windows — most in per-frame draw code, causing dozens of allocations per repaint and periodic GC stutter in every screen. Styles are now cached by (font, size, color, style, anchor, wrap) inside BrandTokens, fixing all call sites at once. Cached styles are shared — treat them as immutable (copy with `new GUIStyle(s)` for variants).
